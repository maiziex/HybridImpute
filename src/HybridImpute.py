...
def check_missing_entries_idx_cv(X_incomplete):
...
def check_distribution(X):
    ...
    plt.savefig("hist_of_values_of_the_matrix.eps")
    plt.close()
    return percentile
...
def CrossValidation(X_incomplete,percentile):
    ...
    m,n = X_incomplete.shape    
    missing_entries_idx_cv = check_missing_entries_idx_cv(X_incomplete)
...
def HybridImpute(X_incomplete,*positional_parameters, **keyword_parameters):
    ...
    percentile = check_distribution(X_incomplete)
